...
def preprocess_data():
    ...
    for i, pic in enumerate(os.listdir('dataset/')):
...
hist = model.fit(X_train, [y_train[0], y_train[1], y_train[2], y_train[3], y_train[4]], batch_size=6, epochs=120,
...
for i, pic in enumerate(os.listdir('dataset/')):
    ...
    if i > 970:
        ...
        pr = predict(os.path.join('dataset/', pic))
        pic_target = pic[:-4]
        if pr == pic_target:
            c = c + 1
print((c / d) * 100)
